        ...
        fetch(config.apiURL+"products/"+config.operatorId, requestOptions).then((response) => {
            ...
            if(productList.length === 0){
                ...
                rowData = productList.map(p => {
                    let button;
                    if(p.active){
                        button = <button className="btn btn-danger" onClick={() => disable(p)}><i className="fas fa-eye-slash"></i> Deshabilitar producto</button>;
                    } else {
                        button = <button className="btn btn-success" onClick={() => enable(p)}><i className="fas fa-eye"></i> Habilitar producto</button>;
                    }
                    ...
                    return (<tr>
                        ...
                        <td className="d-flex justify-content-between">
                            {button}
                            <button className="btn btn-warning" onClick={() => edit(p)}><i className="fas fa-pencil"></i> Editar producto</button>
                        </td>
                    </tr>); 
                });
            }
    ...
    const disable = (product) => {
        if(window.confirm("¿Está seguro/a de querer deshabilitar el siguiente producto: "+product.name+"?")){
            ...
            fetch(config.apiURL+"products/"+product.id, requestOptionsPatch).then((response) => {
                ...
                updateProducts();
                window.alert("El producto ha sido deshabilitado de forma correcta.");
            });   
        }
    }
    const enable = (product) => {
        if(window.confirm("¿Está seguro/a de querer volver a habilitar el siguiente producto: "+product.name+"?")){
            ...
            fetch(config.apiURL+"products/"+product.id, requestOptionsPatch).then((response) => {
                ...
                updateProducts();
                window.alert("El producto ha sido habilitado de forma correcta.")
            });   
        }
    }
    const edit = (product) => {
